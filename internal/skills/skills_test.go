...
func TestDiscoverBuiltin(t *testing.T) {
	...
	for _, s := range discovered {
		if s.Name == "crush-overview" {
			foundOverview = true
			require.Equal(t, "crush://skills/crush-overview/SKILL.md", s.SkillFilePath)
			require.Equal(t, "crush://skills/crush-overview", s.Path)
			require.NotEmpty(t, s.Description)
			require.NotEmpty(t, s.Instructions)
			require.True(t, s.Builtin)
			require.True(t, s.UserInvocable)
		}
	}
	require.True(t, foundOverview, "crush-overview builtin skill not found")
}
