...
type Options struct {
	...
	APIRetryProxyPrefix       string       `json:"api_retry_proxy_prefix,omitempty" jsonschema:"description=Retry failed provider API requests once through this trusted proxy prefix. Retried requests include the full request URL\\, headers\\, and body,example=https://no-cors.deno.dev/"`
